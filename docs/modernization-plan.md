...
- [x] **Switch Pages to GitHub Actions** (done: the first deploy from `main` succeeded on 2026-09-14). Still open: Settings → Environments → `github-pages` → Deployment branches: add `refactor/*` (or "No restriction"), otherwise the manual deploy of a feature branch is rejected by the environment's default `main`-only rule.
- [x] **Merge `refactor/04-deploy` into `main`** (PR #1, merged 2026-09-14; the deploy workflow ran green). The new app is live at https://teefaxx.github.io/trip-consumption-analysis/. If the map shows the placeholder, the `MAPBOX_TOKEN` secret is missing: add it and re-run the workflow on `main`.
...
None at the moment. All decisions are listed at the top of this document.
